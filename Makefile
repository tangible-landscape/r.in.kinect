...
EXTRA_CFLAGS = -Wall -fsanitize=address -O2 -g -std=c++20 -march=native  -Wno-deprecated -O0 $(VECT_CFLAGS) $(OMPCFLAGS) $(ORBBEC_LIB_PATH) $(ORBBEC_LIB) -static-libasan

LDFLAGS += $(ORBBEC_LIB_PATH) $(ORBBEC_LIB)

include $(MODULE_TOPDIR)/include/Make/Module.make

LINK = $(CXX) -fsanitize=address -O2
