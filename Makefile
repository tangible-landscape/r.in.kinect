...
# -lob_usb -ldepthengine -llive555

PGM = r.in.kinect

LIBES = $(IMAGERYLIB) $(RASTERLIB) $(INTERPFLLIB) $(QTREELIB) $(QTREEDEP) $(GMATHLIB) $(INTERPDATALIB) $(VECTORLIB) $(DBMILIB) $(GISLIB) $(OMPLIB) $(ORBBEC_LIB) -rdynamic -lpcl_common -Wl,-Bstatic -lflann_cpp_s -Wl,-Bdynamic -lpcl_io -lpcl_kdtree -lpcl_octree -lpcl_search -lpcl_surface -lpcl_sample_consensus  -lpcl_filters -lpcl_features -lpcl_keypoints  -lpcl_segmentation -lpcl_tracking -lpthread -lboost_system -Wl,-Bdynamic  -ldl -Wl,-rpath,/usr/local/lib
...
EXTRA_CFLAGS = -Wall -g -std=c++20 -march=native -Wno-deprecated -O0 $(VECT_CFLAGS) $(OMPCFLAGS) $(ORBBEC_LIB_PATH) $(ORBBEC_LIB) -static-libasan

LDFLAGS += $(ORBBEC_LIB_PATH) $(ORBBEC_LIB) -L/usr/local/lib
